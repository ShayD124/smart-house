...
export default function RoomPage() {
  ...
  const { rooms, addDevice, toggleDeviceState } = useContext(RoomContext)
  ...
  const handleAddDevice = () =>{
    ...
    let stereoSystemExists = false
    for(let i = 0; i < room.devices.length; i++){
      if(room.devices[i].name === 'Stereo System'){
        stereoSystemExists = true
        break
      }
  ...
  return (
    <div className='flex flex-col items-center p-6 bg-gray-100'>
      ...
      <div className='mb-4 w-full max-w-md'>
        ...
        <div>
          <button onClick={handleAddDevice} className='bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-700 transition-colors duration-300'>Add device</button>
